...
django_root = os.path.abspath(os.path.join(os.path.dirname(__file__),
		   '..', '..', '..', '..'))
venv_python_dir = glob.glob(
    os.path.join(django_root, "env", "lib", "python*")
)[0]
venv = os.path.join(venv_python_dir, "site-packages")
...
os.environ['DJANGO_SETTINGS_MODULE'] = 'conf.noether.settings'
